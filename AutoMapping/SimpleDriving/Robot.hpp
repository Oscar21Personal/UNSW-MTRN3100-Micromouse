...
class Robot {
  public:
    ...
    int run(int nodeCoords[arraySize]) {
      ...
      // Move to next node
      if (nodeCoords[actionCount] != -1 && phase != emergencyStopPhase) {
        // Find travel distance and angle
        int nextX {nodeCoords[actionCount]};
        int nextY {nodeCoords[actionCount + 1]};
        float targetDistance {0.92*sqrt(pow(nextX - currentX, 2) + pow(nextY - currentY, 2))};
        float targetAngle {};
